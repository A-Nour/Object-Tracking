...
classdef GaussianDensity
    ...
    methods (Static)
        ...
        function predicted_likelihood = predictedLikelihood(state_pred,z,measmodel)
            ...
            for meas = z
                predicted_likelihood(meas_idx, 1) = log_mvnpdf(meas', hx', S);
                meas_idx = meas_idx + 1;
            end
        end
        ...
        function [z_ingate, meas_in_gate] = ellipsoidalGating(state_pred, z, measmodel, gating_size)
            ...
            meas_in_gate = [];
            for meas = z
                maha_dist = ((meas - measmodel.h(state_pred.x))'*(inv(S))*(meas - measmodel.h(state_pred.x)));
                if maha_dist < gating_size
                    meas_in_gate(meas_index) = true;
                    z_ingate(:, z_index) = meas';
                    z_index = z_index + 1;
                else
                    meas_in_gate(meas_index) = false;
                end
        ...
        function state = momentMatching(w, states)
            ...
            w = exp(w);
            x = zeros(size(states(1).x));
            P = zeros(size(states(1).P));
            for i = 1 : size(w)
                x = x + w(i) * states(i).x; 
            end
            
            for i = 1 : size(w)
                P = P + w(i) * states(i).P + w(i) * (x - states(i).x) * (x - states(i).x)';
            end
